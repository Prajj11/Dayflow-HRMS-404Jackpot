...
func setAuthCookie(w http.ResponseWriter, token string) {
	...
	http.SetCookie(w, &http.Cookie{Name: authCookieName, Value: token, Path: "/", MaxAge: int(accessTokenTTL.Seconds()), HttpOnly: true, Secure: secure, SameSite: sameSite})
}

func clearAuthCookie(w http.ResponseWriter) {
	secure, sameSite := cookieSecurityAttrs()
	http.SetCookie(w, &http.Cookie{Name: authCookieName, Value: "", Path: "/", MaxAge: -1, HttpOnly: true, Secure: secure, SameSite: sameSite})
}
