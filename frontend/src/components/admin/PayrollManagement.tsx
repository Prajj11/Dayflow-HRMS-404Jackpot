      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Calculated across {employees.length} employee profiles from current salary structures.
          </p>
        </div>
      </div>
          ...
          <div className="w-full max-w-lg rounded-2xl border border-border bg-card p-6 shadow-2xl">
            <div className="flex justify-between items-start border-b border-border pb-4 mb-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Pay Period: {new Date().toLocaleDateString(undefined, { month: "long", year: "numeric" })}</p>
              </div>
              <button
